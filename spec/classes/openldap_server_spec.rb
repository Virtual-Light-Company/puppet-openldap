  ...
  context 'with openldap::client class included' do
    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'with memberof enabled', :compile do
          let(:params) do
            super().merge(
              {
                :memberof     => true,
          ...
          it { should contain_openldap('olcOverlay={0}memberof,olcDatabase={2}hdb,cn=config').with_attributes(
            {
              'objectClass'  => [
                'olcOverlayConfig',
              ],
        ...
        context 'with refint enabled and single attribute', :compile do
          let(:params) do
            super().merge(
              {
                :refint            => true,
          ...
          it { should contain_openldap('olcOverlay={0}refint,olcDatabase={2}hdb,cn=config').with_attributes(
            {
              'objectClass'  => [
                'olcOverlayConfig',
                'olcRefintConfig',
        ...
        context 'with refint enabled and multiple attributes', :compile do
          let(:params) do
            super().merge(
              {
                :refint            => true,
                :refint_attributes => [ 'member', 'member2' ]
              }
            )
          end
